    ...
    if (!isError && ok && parsed.contains(QLatin1String("data"))) {
        ...
        if (paging.contains("next")) {
            ...
            TRACE(SOCIALD_INFORMATION,
                  QString(QLatin1String("Facebook contact sync with account %1 finished with result: %2: a: %3 m: %4 r: %5. Continuing to load avatars..."))
...
bool FacebookContactSyncAdaptor::storeToLocal(const QString &accessToken, int accountId, int *addedCount, int *modifiedCount, int *removedCount)
{
    // steps:
