...
#include "dropboxsignonsyncadaptor.h"
#include "trace.h"
...
DropboxSignonSyncAdaptor::DropboxSignonSyncAdaptor(QObject *parent)
    : DropboxDataTypeSyncAdaptor(SocialNetworkSyncAdaptor::Signon, parent)
{
    setInitialActive(true);
}

DropboxSignonSyncAdaptor::~DropboxSignonSyncAdaptor()
{
}

QString DropboxSignonSyncAdaptor::syncServiceName() const
{
    return QStringLiteral("dropbox-sync"); // TODO: change name of service to dropbox-signon!
}

void DropboxSignonSyncAdaptor::sync(const QString &dataTypeString, int accountId)
{
    // call superclass impl.
    DropboxDataTypeSyncAdaptor::sync(dataTypeString, accountId);
}

void DropboxSignonSyncAdaptor::purgeDataForOldAccount(int, SocialNetworkSyncAdaptor::PurgeMode)
{
    // Nothing to do.
}

void DropboxSignonSyncAdaptor::finalize(int)
{
    // nothing to do
}

void DropboxSignonSyncAdaptor::beginSync(int accountId, const QString &accessToken)
{
    Q_UNUSED(accessToken);
    refreshTokens(accountId);
}

Accounts::Account *DropboxSignonSyncAdaptor::loadAccount(int accountId)
{
    ...
    if (m_accounts.contains(accountId)) {
        ...
        if (!acc) {
            SOCIALD_LOG_ERROR(
                    QString(QLatin1String("error: Dropbox account %1 was deleted during signon refresh sync"))
                    .arg(accountId));
            return 0;
        } else {
    ...
    if (!srv.isValid()) {
        SOCIALD_LOG_ERROR(
                QString(QLatin1String("error: invalid service %1 specified for refresh sync with Dropbox account: %2"))
...
void DropboxSignonSyncAdaptor::raiseCredentialsNeedUpdateFlag(int accountId)
{
    ...
    if (acc) {
        ...
        acc->setValue(QStringLiteral("CredentialsNeedUpdateFrom"), QVariant::fromValue<QString>(QString::fromLatin1("sociald-dropbox-signon")));
        acc->selectService(Accounts::Service());
        acc->syncAndBlock();
    }
}

void DropboxSignonSyncAdaptor::lowerCredentialsNeedUpdateFlag(int accountId)
{
    Accounts::Account *acc = loadAccount(accountId);
    if (acc) {
...
void DropboxSignonSyncAdaptor::refreshTokens(int accountId)
{
    Accounts::Account *acc = loadAccount(accountId);
    if (!acc) {
        return;
    ...
    if (!identity) {
        SOCIALD_LOG_ERROR(
                QString(QLatin1String("error: Dropbox account %1 has no valid credentials, cannot perform refresh sync"))
                .arg(accountId));
        return;
    }
    ...
    if (!accSrv) {
        SOCIALD_LOG_ERROR(
                QString(QLatin1String("error: Dropbox account %1 has no valid account service, cannot perform refresh sync"))
                .arg(accountId));
    ...
    if (!session) {
        SOCIALD_LOG_ERROR(
                QString(QLatin1String("error: could not create signon session for Dropbox account %1, cannot perform refresh sync"))
                .arg(accountId));
        accSrv->deleteLater();
        identity->deleteLater();
        return;
...
void DropboxSignonSyncAdaptor::initialSignonResponse(const SignOn::SessionData &responseData)
{
...
void DropboxSignonSyncAdaptor::forceTokenExpiryResponse(const SignOn::SessionData &)
{
    SignOn::AuthSession *session = qobject_cast<SignOn::AuthSession*>(sender());
    session->disconnect(this);
...
void DropboxSignonSyncAdaptor::triggerRefresh()
{
...
void DropboxSignonSyncAdaptor::signonError(const SignOn::Error &error)
{
    SignOn::AuthSession *session = qobject_cast<SignOn::AuthSession*>(sender());
    int accountId = session->property("accountId").toInt();
    session->disconnect(this);
    ...
    SOCIALD_LOG_INFO(
            QString(QLatin1String("got signon error when performing signon refresh for Dropbox account %1: %2: %3.  Raising flag? %4"))
